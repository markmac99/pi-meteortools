...
def createHtmlIdx(vid_or_img, ym, cam, fldr, buck, s3):
    ...
    with os.fdopen(fd, 'w') as outf:
        ...
        outf.write('fetch("https://echo.zuplo.io/", {\n')
        outf.write('    method: "PUT",\n')
        outf.write('    body: "Very interesting: " + result\n')
        outf.write('}).then((response) => response.text())\n')
        outf.write('.then((responseText) => {\n')
        outf.write('    alert(responseText);\n')
        outf.write('})\n')
        outf.write('.catch((error) => {\n')
        outf.write('    console.error("foo: " + error)\n')
        outf.write('})}\n')
        outf.write('</script>\n')
